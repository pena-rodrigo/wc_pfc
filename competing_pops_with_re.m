...
%   
    ...
    [pkvl,pkid] = findpeaks(r1);
    pkvl = pkvl.*(pkvl>0.3);
    pks1 = sum(pkvl)/t;
    
    [pkvl,pkid] = findpeaks(r2);
    pkvl = pkvl.*(pkvl>0.3);
    pks2 = sum(pkvl)/t;
    ...
    acum1sum = [acum1sum; pks1];% max(r1(end/2:end)) - min(r1(end/2:end))];% sum(r1)];
    acum2sum = [acum2sum; pks2];% max(r2(end/2:end)) - min(r2(end/2:end))];% sum(r2)];
...
figure;
% subplot(1,2,1)
plot(1:1:40,smooth(acum1),'b','LineWidth',3); 
hold on; 
plot(1:1:40,smooth(acum2),'r','LineWidth',3);
...
% subplot(1,2,2)
% plot(acum1sum,'b','LineWidth',2); 
% hold on; 
% plot(acum2sum,'r','LineWidth',2);
% legend('variable','f=20Hz')
...
subplot(1,2,1)
hold on;
plot(1:1:40,acum1,'LineWidth',2); 
subplot(1,2,2)
 hold on;
plot(1:1:40,acum2,'LineWidth',2);
% 